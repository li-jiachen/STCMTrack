...
common_options=("--run_id" "$run_id" "--output_dir" "$output_dir" "--project_name" "$project_name" "--exp_name" "$exp_name")
...
if [[ "$do_sweep" == false ]]; then
    ...
    PYTHONUNBUFFERED=1 OMP_NUM_THREADS=1 python3 main.py "${target_options[@]}" "${common_options[@]}" |& tee -a "$output_log"
else
    ...
    PYTHONUNBUFFERED=1 OMP_NUM_THREADS=1 python3 sweep.py "${target_options[@]}" "${sweep_options[@]}" "${common_options[@]}" |& tee -a "$output_log"
fi
