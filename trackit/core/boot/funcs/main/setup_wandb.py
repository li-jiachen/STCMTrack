...
def setup_wandb(args, network_config: dict, notes: str, extra_tags: list = None):
    try:
        ...
        raise ModuleNotFoundError("wandb is required only when logging is enabled. Install it or rerun with --disable_wandb.") from exc
    tags = None
    ...
    wandb_instance = wandb.init(project=project, name=exp_name, tags=tags, config=config, force=True, job_type='train', id=run_id, mode=mode, dir=output_dir, group=group, notes=notes)
