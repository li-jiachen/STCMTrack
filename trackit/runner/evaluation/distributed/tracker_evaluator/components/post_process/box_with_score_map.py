...
class PostProcessing_BoxWithScoreMap(TrackerOutputPostProcess):
    def __init__(self, device: torch.device,
                 ...
                 classification_score_with_penalty: bool = False,
                 confidence_source: str = 'score_map'):
        ...
        if confidence_source not in ('score_map', 'evidence'):
            raise ValueError(f'Unknown confidence source: {confidence_source}')
        self._confidence_source = confidence_source
    ...
    def __call__(self, output):
        ...
        N, H, W = dense_score_map.shape
        predicted_score_map = dense_score_map.view(N, H * W)
        dense_bbox = (dense_bbox.view(N, H, W, 2, 2) *
        ...
        if self._confidence_source == 'evidence':
            if 'evidence' not in output:
                raise RuntimeError('confidence_source=evidence, but model output does not contain evidence logits')
            confidence_score = output['evidence'].detach().float().sigmoid()
        else:
