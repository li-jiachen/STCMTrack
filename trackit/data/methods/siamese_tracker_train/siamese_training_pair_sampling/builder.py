...
def build_SiamFC_training_pair_sampler(datasets: Sequence[TrackingDataset], datasets_sampling_weight: np.ndarray,
    ...
    if 'negative_sample' in siamese_sampling_config:
        negative_sample_config = siamese_sampling_config['negative_sample']
        negative_sample_weight = negative_sample_config['weight']
        strict_negative_sample = negative_sample_config.get('strict', False)
        negative_sample_max_retry_count = negative_sample_config.get('max_retry_count', 32)

        negative_sample_methods_weight = []

        negative_sample_methods_config = negative_sample_config.get('methods', siamese_sampling_config.get('methods', []))
        for negative_sample_generation_rule in negative_sample_methods_config:
        ...
        strict_negative_sample = False
        negative_sample_max_retry_count = 32
    ...
    return SiamFCTrainingPairSampler(datasets, datasets_sampling_weight, sequence_picker,
                                     ...
                                     strict_negative_sample=strict_negative_sample,
                                     negative_sample_max_retry_count=negative_sample_max_retry_count,
                                     num_template_frames=siamese_sampling_config['positive_sample']['num_template_frames'],
                                     num_search_frames=siamese_sampling_config['positive_sample']['num_search_frames'],
                                     max_sample_interval=siamese_sampling_config['positive_sample']['MAX_SAMPLE_INTERVAL'])
