#!/usr/bin/env python3
"""AUC / P@20 / P_n evaluation for Anti-UAV410 and Anti-UAV (IR_label.json ground truth).

Per sequence, over frames whose ground-truth box has positive width and height:
  * AUC  - area under the success curve, IoU thresholds 0:0.01:1 (IoU >= threshold);
  * P@20 - fraction of frames with center error <= 20 pixels;
  * P_n  - fraction of frames with center error / sqrt(w_gt * h_gt) <= 0.5.
The reported numbers are the unweighted mean over sequences.
"""
...
_trapezoid = getattr(np, 'trapezoid', None) or getattr(np, 'trapz')  # np.trapz was removed in NumPy 2.4


def calculate_auc(success_rates: np.ndarray, thresholds: np.ndarray) -> float:
    return float(_trapezoid(success_rates, thresholds) / (thresholds[-1] - thresholds[0]))
