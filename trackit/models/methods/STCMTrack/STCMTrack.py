...
from .modules.ltcp import TemporalTokenContextPropagation, TemporalTokenContextPropagationConfig
...
class STCMTrack_DINOv2(nn.Module):
    def __init__(self, vit: DinoVisionTransformer,
                 ...
                 route_compression: bool = False, evidence_enabled: bool = False,
                 evidence_train_only: bool = False, ltcp_config: Optional[dict] = None):
        super().__init__()
        ...
        # Due to the random initialization of the trackit framework, sometimes the losses do not converge. Please try restarting the experiment several times
        ...
        elif evidence_enabled and evidence_train_only:
            self._freeze_except_evidence_head()

    def _freeze_except_evidence_head(self):
        for name, param in self.named_parameters():
            param.requires_grad = name.startswith('head.evidence_mlp.')
    ...
    def load_state_dict(self, state_dict: Mapping[str, Any], **kwargs):
        if 'expert_alpha' in state_dict:
            ...
            self.expert_alpha = state_dict['expert_alpha'].item()
            self.use_rsexpert = state_dict['use_rsexpert'].item()
