...
class MlpAnchorFreeHead(nn.Module):
    def __init__(self, dim: int, map_size: Tuple[int, int], evidence_enabled: bool = False):
        ...
        if evidence_enabled:
            self.evidence_mlp = Mlp(dim, out_features=1, num_layers=2)
    ...
    def forward(self, x):
        ...
        output = {'score_map': score_map, 'boxes': box_map}
        if self.evidence_enabled:
            evidence = self.evidence_mlp(x.mean(dim=1)).to(torch.float32).view(B)
