...
class SiamTrackerTrainingPairProcessorBatchCollator:
    ...
    def __call__(self, batch: Sequence[Mapping], collated: TrainData):
        ...
        for i in range(max_x + 1):
            data_dict[f'x_{i}'] = collate_element_as_torch_tensor(batch, f'x_{i}_cropped_image')
        for i in range(max_z + 1):
            data_dict[f'z_{i}'] = collate_element_as_torch_tensor(batch, f'z_{i}_cropped_image')
