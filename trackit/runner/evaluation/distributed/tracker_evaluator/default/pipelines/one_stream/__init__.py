...
class OneStreamTracker_Evaluation_MainPipeline(TrackerEvaluationPipeline):
    ...
    def _append_memory_frame(self, task_id, memory_frame: torch.Tensor):
        ...
        if len(memory_frames) <= self.memory_frame_limit:
            return
        # Bound the history to memory_frame_limit templates: the first-frame template and the oldest entries are kept,
        # and once the history is full each new template replaces the newest entry.
    ...
    def prepare_tracking(self, context, model_input_params):
            ...
            if task.tracker_do_tracking_context is not None:
                ...
                if self.ctr is not None:
                    ...
                    if geometric_bbox is not None:
                        cropping_params_provider.update(None, geometric_bbox, image_size)
        ...
        for i, task_id in enumerate(task_ids):
            if len(self.memory_frames[task_id]) == 1:
                ...
                selected_indexes = self.select_memory_frames(len(self.memory_frames[task_id]), dataset)
                for i_selected, idx_selected in enumerate(selected_indexes):
    ...
    def select_memory_frames(self, num_memory_frames, dataset_name):
        # Templates: the first-frame template z plus two templates from the template history (positions N/4 and 3N/4).
        num_segments = 2
    ...
    def on_tracked(self, model_outputs, context):
        ...
        for index, (task_id, image_size, frame_index) in enumerate(zip(task_ids, x_frame_sizes, x_frame_indices)):
            ...
            # Crop a template from the final box and append it to the template history.
            ...
            if not bbox_is_valid(memory_bounding_box_on_full_search_image):
                ...
                continue
            template_curation_parameter = get_siamfc_cropping_params(memory_bounding_box_on_full_search_image, 2.0, np.array([196, 196]))
