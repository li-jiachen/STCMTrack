...
from typing import Optional, Union, Callable, Mapping, Any, Sequence, MutableSequence, List, Protocol
...
class ModelManager:
    ...
    def load_state_dict(self, state_dict: dict, strict: bool = False, print_missing: bool = True):
        if self._newest_model is None:
            ...
            self._version += 1
            self._sync_cached_newest_model_instances()
    ...
    def _sync_cached_newest_model_instances(self):
        for model_instance in tuple(self._model_instance_cache.values()):
            if model_instance._model is self._newest_model:
                model_instance._version = self._version
